              ...
              {!rollLoading ? rollCall.players.map((player) => {
                ...
                return (
                  <article className="rollcall-player" key={player.id}>
                    <div className="rollcall-player-id"><span className="rollcall-avatar">{initials(playerName(player))}</span><span><strong>{playerName(player)}</strong><small>{player.dorsal != null ? `#${player.dorsal}` : 'Sin dorsal'} · {player.position || 'Sin posición'} {response ? `· ${response === 'yes' ? 'Confirmó asistencia' : 'Avisó ausencia'}` : ''}</small></span></div>
