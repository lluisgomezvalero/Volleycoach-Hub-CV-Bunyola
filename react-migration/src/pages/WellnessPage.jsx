...
export default function WellnessPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const from28Iso = new Date(Date.now() - 28 * 86400000).toISOString();
  ...
  const loadsByPlayer = useMemo(() => {
    ...
    const sevenCutoff = Date.now() - 7 * 86400000;
    const map = new Map(players.map((player) => [player.id, { seven: 0, twentyEight: 0, sessions: 0 }]));

    rpeRows.filter((row) => row.source === 'player' && row.player_id).forEach((row) => {
      ...
      const minutes = Number.isFinite(effectiveMinutes) && effectiveMinutes > 0 ? effectiveMinutes : eventDuration(event);
      const load = Math.round(Number(row.score || 0) * minutes);
      const target = map.get(row.player_id) || { seven: 0, twentyEight: 0, sessions: 0 };
      target.twentyEight += load;
      target.sessions += 1;
      if (new Date(event.starts_at).getTime() >= sevenCutoff) target.seven += load;
      map.set(row.player_id, target);
    });

    map.forEach((value) => {
      const chronicWeek = value.twentyEight / 4;
      value.ratio = chronicWeek > 0 ? value.seven / chronicWeek : null;
      value.state = loadState(value.ratio, value.seven);
    });
    return map;
      ...
      <div className="wellness-page wellness-player-page">
        ...
        <section className="wellness-card wellness-player-load-card">
          ...
          <div className={`wellness-load-status wellness-load-${currentLoad?.state?.key || 'neutral'}`}>
            <Gauge size={17} />
            <div><strong>{currentLoad?.state?.label || 'Sin referencia'}</strong><span>Se calcula a partir de tus entrenamientos y tu RPE.</span></div>
          </div>
        </section>
        ...
        <div className="wellness-modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) setSelectedPlayerId(null); }}>
          <section className="wellness-detail-modal">
            ...
            <div className="wellness-detail-load">
              ...
              <WellnessMetric label="Carga 28 días" value={Math.round(selectedModel.load.twentyEight)} suffix=" UA" />
            ...
            <div className={`wellness-load-status wellness-load-${selectedModel.load.state.key}`}>
              <Info size={17} /><div><strong>{selectedModel.load.state.label}</strong><span>Referencia descriptiva de carga; no equivale por sí sola a riesgo de lesión.</span></div>
            </div>
