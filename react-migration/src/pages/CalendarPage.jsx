...
function EventCard({ event, leagueTeams, onOpen }) {
      ...
      <div className="calendar-layout">
        ...
        <section className="calendar-agenda-card">
          ...
          <div className="calendar-agenda-list">
            {loading ? <div className="calendar-empty-state"><span className="calendar-loading-dot" /><strong>Cargando calendario…</strong></div> : selectedEvents.length ? selectedEvents.map((event) => <EventCard key={event.id} event={event} leagueTeams={leagueTeams} onOpen={setDetailEvent} />) : <div className="calendar-empty-state"><CalendarCheck size={24} /><strong>Sin eventos este día</strong><span>{isStaff ? 'Puedes añadir un evento directamente en esta fecha.' : 'Pulsa otra fecha para consultar su agenda.'}</span>{isStaff ? <button type="button" className="calendar-empty-add" onClick={openCreate}><Plus size={17} />Añadir evento</button> : null}</div>}
          </div>
        </section>
      </div>
