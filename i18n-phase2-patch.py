...
marker = "  ['Seguimiento semanal','Seguiment setmanal'],['Pendientes','Pendents'],['Sin pendientes','Sense pendents'],['Sin datos','Sense dades'],['No hay datos disponibles','No hi ha dades disponibles']\n];"
replacement = """  ['Seguimiento semanal','Seguiment setmanal'],['Pendientes','Pendents'],['Sin pendientes','Sense pendents'],['Sin datos','Sense dades'],['No hay datos disponibles','No hi ha dades disponibles'],
...
old_elements = "  const elements=root.querySelectorAll?root.querySelectorAll('[title],[aria-label],[placeholder]'):[];\n  elements.forEach(el=>{"
new_elements = """  const elements=[];
  ...
  elements.forEach(el=>{"""
if old_elements not in s:
    raise SystemExit('attribute translation block not found')
s = s.replace(old_elements, new_elements, 1)

pattern = re.compile(r"function scheduleApply\(\)\{.*?\n\}\n\nfunction install\(\)\{.*?\n\}\n\nwindow\.VolleyI18n", re.S)
new_tail = r"""const pendingRoots=new Set();
...
window.VolleyI18n"""
s2, n = pattern.subn(new_tail, s, count=1)
if n != 1:
    raise SystemExit(f'observer block replacement count={n}')
s = s2
path.write_text(s, encoding='utf-8')

cfg = Path('supabase-config.js')
c = cfg.read_text(encoding='utf-8')
c = c.replace("window.VOLLEY_ASSET_VERSION = '20260825ux1';", "window.VOLLEY_ASSET_VERSION = '20260826i18n2';")
c = c.replace("'i18n-es-ca-20260826.js?v=20260826i18n1'", "'i18n-es-ca-20260826.js?v=20260826i18n2'")
if '20260826i18n2' not in c:
    raise SystemExit('cache version replacement failed')
